...
import { registerScheme, type RegisterScheme } from "../../lib/schemas/registerScheme";


export default function RegisterForm() {
  const { registerUser } = useAccount();
  const {control, handleSubmit, setError, formState: {isValid, isSubmitting}} = useForm<RegisterScheme>({
    mode: 'onTouched',
  ...
  const onSubmit = async (data: RegisterScheme) => {
    await registerUser.
    mutateAsync(data, {onError: (error) =>{
    ...
    sx={{display:'flex', flexDirection: 'column', p: 3, gap: 3, maxWidth: 'md', mx:'auto', borderRadius: 3}}>
        ...
        <Typography sx={{textAlign: 'center'}}>Already have an account?
          <Typography sx={{ml: 2}} component={Link} to='/login' color="primary">Sing in</Typography>
        </Typography>
    </Paper>
  )
}